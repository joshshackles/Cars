...
- `CARS_SKIP_DB_BOOTSTRAP`: optional escape hatch. Vercel builds already skip database bootstrap unless `CARS_AUTO_DB_BOOTSTRAP=true`.
- `CARS_AUTO_DB_BOOTSTRAP`: set to `true` only for a deliberate one-off database bootstrap/schema sync run.
- `CARS_ACCEPT_DB_PUSH_WARNINGS`: set to `true` only when intentionally allowing Prisma `db push --accept-data-loss` warnings after reviewing the database impact.
...
5. Initialize or update the Neon schema from a trusted environment, not as part of every Vercel build. For the current MVP schema-sync path, run `CARS_AUTO_DB_BOOTSTRAP=true npm run vercel-build -- --db-only`. If Prisma reports a reviewed warning, rerun with `CARS_ACCEPT_DB_PUSH_WARNINGS=true`.
6. Deploy to Vercel. Normal Vercel builds generate Prisma Client and run `next build`; they do not mutate the database unless `CARS_AUTO_DB_BOOTSTRAP=true` is explicitly set.
7. For mature production releases, replace the bootstrap push with committed Prisma migrations and run `npm run prisma:deploy` from a trusted release environment.
8. Optionally seed demo data only with `ALLOW_PRODUCTION_SEED=true npm run db:seed`.
9. Verify production health at `/api/health`.
